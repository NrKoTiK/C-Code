...
namespace seneca
{
	...
	// --Private Members--
	Time PreTriage::getWaitTime(const Patient& PwTime) const
	{
		int count = 0;
		for (int i = 0; i < m_numOfP; ++i)
		{
			if (PwTime.type() == m_PinLine[i]->type())
			{
				++count;
			}
		}
		if (PwTime.type() == 'C')
		{
			Time C(static_cast<unsigned int>(m_avgTimeC) * count);
			return C;
		}
		if (PwTime.type() == 'T') 
		{
			Time T(static_cast<unsigned int>(m_avgTimeT) * count);
			return T;
		}
		return Time();
	}
	...
	void PreTriage::registerp()
	{
		...
		if (choice == 1)
		{
			...
			std::cout << getWaitTime(*TP) << std::endl;
			std::cout << "******************************************\n\n";
			m_PinLine[m_numOfP] = TP;
			++m_numOfP;
		}
		else if (choice == 2)
		{
			TriagePatient* TP = new TriagePatient();
			TP->setArrivalTime();
			std::cout << "Please enter patient information: " << std::endl;
			std::cin.ignore(1000, '\n');
			std::cin >> *TP;
