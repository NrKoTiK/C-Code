...
void displayFile(const char* filename) {
   ...
   char ch;
   while (file.get(ch) && std::cout << ch); // This is only getting 3 lines it is not getting the last line of char from file
   std::cout << "---------------------------------" << endl;
}
...
int main() {
   ...
   while (csvfile >> W) {
      p[i++] = W;
         csvoutfile << W << endl; //This maybe fualt ??
   }
...
void displayAll(const Patient* p, size_t num) { // Data is not being called here at all. it gets erased and the last function that is called before this is the sorting function.
